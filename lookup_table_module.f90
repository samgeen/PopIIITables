...
SUBROUTINE read_table(table)
  ...
  IF (.not.(table%contains_data)) THEN
     ...
     read(1336)table%axis1
     read(1336)table%axis2
     read(1336)table%axis3
     read(1336)table%axis4
...
! numaxis - number of elements in the axis
  ...
  io = MIN(io,numaxis-1) ! Must be -1 as we add 1 to interpolate later
  io = MAX(io,1)
  ! Debug message
  IF (debug_lkup) &
